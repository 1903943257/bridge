...
def _make_model(device, dtype, *, tpr, max_sequence_length=_MAX_SEQUENCE_LENGTH):
    ...
    model_spec = replace_self_attention_with_tpr(original_spec) if tpr else original_spec
    model = GPTModel(
        config=config,
    ...
    model.train()
    expected_type = TPRSelfAttention if tpr else SelfAttention
...
def test_tiny_gpt_model_full_vs_external_kv(prefix_length, suffix_length, monkeypatch):
    ...
    if prefix_length:
        ...
        _assert_finite(prefix_logits, "TPR prefix logits")
    ...
    assert tpr_suffix_logits.shape == (1, suffix_length, _VOCAB_SIZE)
    _assert_finite(tpr_suffix_logits, "TPR suffix logits")
    ...
    tpr_suffix_logits.backward(upstream_gradient)
    tpr_parameter_grads = _all_parameter_grads(model)
    ...
    assert tpr_parameter_grads.keys() == reference_parameter_grads.keys()
    for name in reference_parameter_grads:
        _assert_gradient_close(
            tpr_parameter_grads[name],
            reference_parameter_grads[name],
            label=f"parameter gradient {name}",
        )


def test_tpr_gpt_model_state_dict_is_strictly_compatible(monkeypatch):
    device = torch.device("npu")
    dtype = torch.bfloat16
    _install_single_rank_test_runtime(monkeypatch, device)
    torch.manual_seed(11)
    ordinary_model = _make_model(device, dtype, tpr=False)
    torch.manual_seed(22)
    tpr_model = _make_model(device, dtype, tpr=True)

    ordinary_state = ordinary_model.state_dict()
    tpr_state = tpr_model.state_dict()
    assert ordinary_state.keys() == tpr_state.keys()
    tpr_model.load_state_dict(ordinary_state, strict=True)
    loaded_state = tpr_model.state_dict()
