...
def _external_kv_step(model, optimizer, input_ids, prefix_length, suffix_length, upstream_gradient):
    ...
    if prefix_length:
        ...
        for layer_number, (key, value) in prefix_context.new_key_values.items():
            _assert_finite(key, f"TPR prefix layer {layer_number} new K")
            _assert_finite(value, f"TPR prefix layer {layer_number} new V")
    ...
    _assert_finite(suffix_logits, "TPR suffix logits")
    for layer_number, (key, value) in suffix_context.new_key_values.items():
        _assert_finite(key, f"TPR suffix layer {layer_number} new K")
        _assert_finite(value, f"TPR suffix layer {layer_number} new V")
    _normalized_backward(suffix_logits, upstream_gradient)
    _copy_model_grads_to_master(
        model, optimizer.param_groups[0]["params_by_name"], phase="TPR"
    )
    optimizer.step()
    for name, parameter in optimizer.param_groups[0]["params_by_name"].items():
        _assert_finite(parameter, f"TPR FP32 master parameter after step {name}")
...
def test_tiny_gpt_single_optimizer_step_matches_external_kv(
    ...
    reference_model = _make_model(
        device, dtype, tpr=True, max_sequence_length=full_length
    )
