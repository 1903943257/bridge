...
"""Final single-rank Engine-level reference comparison for the TPR MVP."""
...
def _make_engine(model, *, tpr_enabled, monkeypatch):
...
def test_engine_tpr_matches_native_megatron_microbatch_accumulation(
    monkeypatch,
    prefix_length,
    suffix_length,
):
    ...
    reference_model = _make_model(device, tpr=False, max_sequence_length=full_length)
    tpr_model = _make_model(device, tpr=True, max_sequence_length=full_length)
    ...
    if reference_oom is not None:
        ...
        del reference_engine, tpr_engine, reference_model, tpr_model
        gc.collect()
        torch.npu.empty_cache()
        pytest.skip(
            f"Reference Engine OOM for P={prefix_length}, S={suffix_length}; "
            "capacity is covered by the separate TPR long-context test"
        )
    ...
    _assert_gradients_close(tpr_gradients, reference_gradients)
