...
def test_segment_push_pop_matches_full_causal_backward(monkeypatch):
    ...
    tpr_gradients = _parameter_grads(model)
    tpr_loss = suffix_result.normalized_loss + prefix_result.normalized_loss

    torch.testing.assert_close(tpr_loss.float(), reference_loss.detach().float(), atol=2e-2, rtol=2e-2)
