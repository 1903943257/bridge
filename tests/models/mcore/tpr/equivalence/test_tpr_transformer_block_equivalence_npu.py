...
def _make_block(device, dtype):
    ...
    tpr_spec = replace_self_attention_with_tpr(original_spec)
    assert all(
        layer.submodules.self_attention.module is not TPRSelfAttention
    ...
    assert all(isinstance(layer.self_attention, TPRSelfAttention) for layer in block.layers)
...
def test_two_layer_transformer_block_full_vs_external_kv(
    ...
    for layer_number, _ in retained_past.items():
        ctx_k, ctx_v = suffix_context.get_past_kv(layer_number)
        assert ctx_k is not None, f"layer {layer_number}: get_past_kv returns None key"
        assert ctx_v is not None, f"layer {layer_number}: get_past_kv returns None value"
    tpr_suffix_output.backward(upstream_gradient)
    tpr_parameter_grads = _all_parameter_grads(block)
