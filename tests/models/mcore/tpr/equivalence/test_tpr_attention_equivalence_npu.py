...
def _make_attention(device, dtype):
    ...
    tpr_layer_spec = replace_self_attention_with_tpr(original_layer_spec)
    assert original_layer_spec.submodules.self_attention.module is not TPRSelfAttention
    attention = build_module(
        tpr_layer_spec.submodules.self_attention,
        config=config,
        layer_number=1,
        pg_collection=SimpleNamespace(tp=process_group, cp=process_group),
    )
    assert isinstance(attention, TPRSelfAttention)
...
def test_real_megatron_attention_full_vs_external_kv(prefix_length, suffix_length, monkeypatch):
    ...
    tpr_parameter_grads = _selected_parameter_grads(attention)
