...
"""Opt-in 0.566B Engine-level TPR correctness report."""
...
from ..profiling import test_tpr_engine_profile_npu as profile
...
pytestmark = pytest.mark.skipif(
    os.getenv("TPR_RUN_CORRECTNESS_REPORT") != "1",
    reason="Set TPR_RUN_CORRECTNESS_REPORT=1 for the 0.566B correctness report",
)
...
def test_tpr_correctness_report(monkeypatch):
        ...
        if segment.loss_terms:
            ...
            for term, value in zip(segment.loss_terms, values, strict=True):
                ...
                if term.sample_id is None:
                    tpr_logprobs[:, global_query] = value
                else:
                    tpr_logprobs[term.sample_id - 1, global_query] = value
    ...
    tu.assign_non_tensor(tpr_data, **{TPR_REQUEST_KEY: TPRForwardBackwardRequest(plan)})
    tpr_output = tpr_engine.forward_backward_batch(tpr_data, loss_function=None, forward_only=False)
    tpr_gradients = _gradient_snapshot(tpr_model)

    assert not torch.isnan(tpr_logprobs).any().item(), "TPR logprob reconstruction is incomplete"
    torch.testing.assert_close(
        tpr_logprobs,
        reference_logprobs_a,
        atol=_LOGPROB_ATOL,
    ...
    tpr_logprob = _logprob_metrics(tpr_logprobs, reference_logprobs_a)
    tpr_gradients = _gradient_metrics(tpr_gradients, reference_gradients_a)
    assert tpr_gradients["relative_l2"] <= _GLOBAL_GRAD_RELATIVE_L2_TOL
    assert tpr_gradients["cosine"] >= _GLOBAL_GRAD_COSINE_MIN
    ...
    tpr_loss_abs = abs(tpr_output["loss"] - reference_loss_a)
    tpr_loss_relative = tpr_loss_abs / max(abs(reference_loss_a), 1e-12)
    ...
    print("                                      Ref A vs Ref B      TPR vs Ref A")
    print(
        "  Logprob relative L2              "
        f"{reference_repeat_logprob['relative_l2']:>14.6e}  {tpr_logprob['relative_l2']:>14.6e}"
    )
    print(
        "  Logprob cosine                   "
        f"{reference_repeat_logprob['cosine']:>14.9f}  {tpr_logprob['cosine']:>14.9f}"
    )
    print(
        "  Logprob max abs diff             "
        f"{reference_repeat_logprob['max_abs']:>14.6e}  {tpr_logprob['max_abs']:>14.6e}"
    )
    print(
        "  Loss relative diff               "
        f"{reference_repeat_loss_relative:>14.6e}  {tpr_loss_relative:>14.6e}"
    )
    print(
        "  Gradient relative L2             "
        f"{reference_repeat_gradients['relative_l2']:>14.6e}  {tpr_gradients['relative_l2']:>14.6e}"
    )
    print(
        "  Gradient cosine                  "
        f"{reference_repeat_gradients['cosine']:>14.9f}  {tpr_gradients['cosine']:>14.9f}"
    )
    print(
        "  Gradient max abs diff            "
        f"{reference_repeat_gradients['max_abs']:>14.6e}  {tpr_gradients['max_abs']:>14.6e}"
    )
    print("Loss values:")
    print(f"  Reference A: {reference_loss_a:.9f}")
    print(f"  Reference B: {reference_loss_b:.9f}")
    print(f"  TPR:         {tpr_output['loss']:.9f}")
    print(f"Gradient tensors compared: {tpr_gradients['parameter_count']}")
    print(
    ...
    print(
        "Worst TPR/Ref gradient tensor: "
        f"{tpr_gradients['worst_name']} ({tpr_gradients['worst_relative_l2']:.6e})"
    )
