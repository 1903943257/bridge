...
"""Opt-in NPU audit of TPR live-storage and phase-level memory peaks."""
...
from . import test_tpr_engine_profile_npu as profile
...
pytestmark = pytest.mark.skipif(
    os.getenv("TPR_RUN_MEMORY_AUDIT") != "1",
    reason="Set TPR_RUN_MEMORY_AUDIT=1 for the NPU memory-lifecycle audit",
)
...
def _build_model(device, *, tpr, max_sequence_length):
...
def _make_tpr_data(prefix_length, suffix_length, sibling_count, device):
...
def _measure_tpr_phases(run, zero_grad):
    rows = []
    originals = {
...
def test_leaf_root_memory_crossover_by_suffix_length(monkeypatch):
    ...
    for suffix_length in _CROSSOVER_SUFFIXES:
        ...
        phases = _measure_tpr_phases(run, zero_grad)
