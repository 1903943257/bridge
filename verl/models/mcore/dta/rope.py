...
"""RoPE construction helpers for suffix-only DTA forwards."""
...
def build_suffix_rotary_pos_emb(
    ...
    *,
    ...
    if not isinstance(rotary_pos_emb, Tensor):
        raise TypeError(
            "DTA RoPE MVP requires standard RotaryEmbedding to return one tensor; "
    ...
    if rotary_pos_emb.shape[1] != 1 or rotary_pos_emb.shape[2] != 1:
        raise ValueError(
            "DTA RoPE MVP requires singleton batch/head broadcast dimensions, "
            f"got {rotary_pos_emb.shape}"
        )
